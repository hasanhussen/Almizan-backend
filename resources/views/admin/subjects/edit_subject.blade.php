...
@section('content')
    <div class="container py-4">
        <h4 class="fw-bold mb-4 text-warning">Edit Subject</h4>
                ...
                <form action="{{ route('subjects.update', $subject->id) }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="year" class="form-select" required>
                                ...
                                @foreach ($years as $year)
                                    <option value="{{ $year }}"
                                        {{ old('year', $subject->year) == $year ? 'selected' : '' }}>
                                        {{ $year }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="semester" class="form-select" required>
                                ...
                                @foreach ($semesters as $semester)
                                    <option value="{{ $semester }}"
                                        {{ old('semester', $subject->semester) == $semester ? 'selected' : '' }}>
                                        {{ ucfirst($semester) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="mark" class="form-control" min="0"
                                value="{{ old('mark', $subject->mark) }}" required>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="success_rate" class="form-control" placeholder="Enter success rate"
                                value="{{ old('success_rate', $subject->success_rate) }}" min="0" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="border rounded p-3" style="max-height: 200px; overflow-y: auto;">
                            @forelse($teachers as $teacher)
                                @php
                                    $checked = is_array(old('teachers'))
                                        ? in_array($teacher->id, old('teachers'))
                                        : $subject->teachers->contains($teacher->id);
                                @endphp
                        ...
                        <small class="text-muted">
                            Select teachers responsible for this subject
                        </small>
                    </div>
                    ...
                    <div class="mt-4 text-end">
                        ...
                        <button type="submit" class="btn btn-warning">
                            <i class="bi bi-pencil-square"></i> Update subject
                        </button>
                    </div>
                </form>
            </div>
